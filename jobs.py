...
PROCESSED_FILE = "/home/anar/Music/anar/gh/PythonScripts/jobs.json"
    ...
    try:
        ...
        for job in job_listings:
            ...
            if title_elem:
                ...
                job_data["url"] = (
                    "https://findajob.dwp.gov.uk" + link["href"] if link else ""
                )
            ...
            if details:
                ...
                for item in list_items:
                    ...
                    # Extract salary
                        ...
                        "£" in text
                        or "per annum" in text.lower()
                        or "per year" in text.lower()
                    ):
...
def main():
    ...
    for job in new_jobs:
        ...
        print()
        print(
            f"  {c.DIM}Location:{c.RESET}  {c.WHITE}{job.get('location', 'N/A')}{c.RESET}"
        )
        print(
            f"  {c.DIM}Salary:{c.RESET}    {c.GREEN}{job.get('salary', 'N/A')}{c.RESET}"
        )
        print(
            f"  {c.DIM}Type:{c.RESET}      {c.YELLOW}{job.get('employment_status', 'N/A')}{c.RESET}"
        )
        print(
            f"  {c.DIM}Hours:{c.RESET}     {c.YELLOW}{job.get('hours', 'N/A')}{c.RESET}"
        )
        if "remote_working" in job:
            print(
                f"  {c.DIM}Remote:{c.RESET}    {c.WHITE}{job['remote_working']}{c.RESET}"
            )
        print()
