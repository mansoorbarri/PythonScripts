...
PROCESSED_FILE = "/home/anar/code/pda.json"
    ...
    for listing in listings:
        ...
        if job_id not in processed_listings:
            ...
            company = listing.find(
                "p", class_="govuk-body govuk-!-margin-bottom-0"
            ).text.strip()
            location = listing.find(
                "p", class_="govuk-body das-!-color-dark-grey"
            ).text.strip()

            wage_elem = listing.find("b", string="Wage")
            wage = (
                wage_elem.parent.get_text(strip=True).replace("Wage", "").strip()
                if wage_elem
                else "Not specified"
            )

            closing_elem = listing.find(
                "p", class_="govuk-body govuk-!-margin-bottom-0 govuk-!-margin-top-1"
            )
            closing_date = (
                closing_elem.text.strip() if closing_elem else "Not specified"
            )
...
def format_for_terminal(apprenticeships):
    ...
    for app in apprenticeships:
        ...
        output += (
            f"  {c.DIM}Closes:{c.RESET}    {c.YELLOW}{app['closing_date']}{c.RESET}\n"
        )
        output += "\n"
